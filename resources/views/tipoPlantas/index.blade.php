...
<body class="flex font-sans h-screen">
    ...
    <main class="flex flex-5 flex-col p-25 bg-[#fbfbfb] h-full gap-4">
        <section class="flex items-center justify-between">
            ...
            <div class="flex-1">
                <a href="{{ route('tipoPlantas.create') }}"
                ...
                <tbody>
                    @foreach ($posts as $tipo)
                        <tr class="border-t-2 border-[#c2c4c7]">
                            <td class="p-4 font-bold text-center">{{ $tipo->id }}</td>
                            <td class="p-4">{{ $tipo->nombre }}</td>
                            <td class="flex gap-6 p-4">
                                <a class="flex flex-1 items-center justify-center gap-3 bg-[#f5f9f0] py-2 text-[#77a856] font-bold rounded-[10px] hover:scale-110 transition duration-300"
                                    href="{{ route('tipoPlantas.edit', $tipo->id) }}"><img class="w-[10%] h-auto"
                                        src="{{ asset('images/editar.png') }}" alt=""> Editar</a>
                                <form action="{{ route('tipoPlantas.destroy', $tipo->id) }}" method="POST"
                                    class="flex flex-1">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        ...
                                        <img class="w-[10%] h-auto" src="{{ asset('images/eliminar.png') }}"
                                            alt=""> Eliminar
                                    </button>
                                </form>
                            </td>
